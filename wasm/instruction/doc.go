...
// and WasmRunner.
//
// Externally each instruction is seen through a simple wasm.Instruction interface.
// Internally they are seen through the instruction.ctxInstruction interface, which
// provides the extra interface methods each object needs to be handles by their
...
//             (unreachable, nop, else, end, drop, select, *.eqz)
package instruction
