...
type ctxInstruction interface {
	...
	makeBlock(instruction wasm.Instruction) []wasm.Instruction
	Read(r *context.Reader)
	Run(vm *Runner)
	setIP(ip int)
	setNext(next wasm.Instruction)
...
func (o *Base) makeBlock(instruction wasm.Instruction) []wasm.Instruction {
	return []wasm.Instruction{instruction}
}
...
func ReadBlock(r *context.Reader) []wasm.Instruction {
	r.IP = 0
...
func readSubBlock(r *context.Reader) []wasm.Instruction {
	block := make([]wasm.Instruction, 0)
	for {
		opcode := r.GetByte()
		if r.Error != nil {
			break
...
func RunBlock(vm *Runner, block []wasm.Instruction) error {
...
func (o *Base) setNext(next wasm.Instruction) {
...
func (o *Base) StackChange(_ *wasm.Module) int {
...
func WriteBlock(w *context.Writer, block []wasm.Instruction) {
